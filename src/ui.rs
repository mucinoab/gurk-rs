...
fn draw_messages<B: Backend>(f: &mut Frame<B>, app: &mut App, area: Rect) {
    ...
    let messages_from_offset = messages.iter().rev().skip(offset).filter_map(|msg| {
        ...
        info!(
            "display message: prefix = {}, width = {}, height = {}, print_receipt = {}",
            prefix, width, height, print_receipt
        );
...
fn display_message(
    ...
    let mut reactions_added = false;
    if !text.is_empty() && !msg.reactions.is_empty() {
        fmt::write(
            &mut text,
            format_args!(
                " [{}]",
    ...
    let mut text = if !msg.reactions.is_empty() {
        format!(
            "{text} [{reactions}] {receipt}",
            text = msg.message.as_ref()?,
            reactions = ,
            receipt = receipt,
        )
    };
    ...
    fn test_display_attachment_only_message() {
        ...
        let msg = Message {
            ...
            attachments: vec![Attachment {
                ...
                filename: "/Users/boxdot/Library/Application Support/gurk/signal-2022-01-16T11:59:58.405665+00:00.jpg".into(),
                size: 238987
            }
        ], reactions: vec![], receipt: Receipt::Sent };

        let res = display_message(&names, &msg, prefix, width, height, print_receipt);
        panic!("{:?}", res);
    }
}
